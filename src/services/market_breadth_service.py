...
class MarketBreadthAnalyzer:
    ...
    def new_high_low_ratio(self) -> Dict:
        ...
        """
        # 1년치 데이터 필요 (조건 완화: 200일 이상이면 분석 가능)
        df = self.get_market_data(days=260)
        
        if df.empty or len(df) < 200:
            return {"error": "데이터 부족 (최소 200일 필요)"}
        
        # 52주 고가/저가 계산 (가용 데이터 범위 내에서)
        available_days = min(len(df), 252)
        high_52w = df.tail(available_days).max()
        low_52w = df.tail(available_days).min()
        current = df.iloc[-1]
